...
ObjectModel::ObjectModel(char* FileName, LoadFiles* FH)
			...
			for (size_t f = 0; f < shapes[s].mesh.indices.size() / 3; f++) {
				...
				for (int k = 0; k < 3; k++) {
					...
					for (int k = 0; k < 3; k++) {
						...
						min.x = std::min(v[k][0], min.x);
						min.y = std::min(v[k][1], min.y);
						min.z = std::min(v[k][2], min.z);
						max.x = std::max(v[k][0], max.x);
						max.y = std::max(v[k][1], max.y);
						max.z = std::max(v[k][2], max.z);
					}
					this->min = min;
					this->max = max;
				}
